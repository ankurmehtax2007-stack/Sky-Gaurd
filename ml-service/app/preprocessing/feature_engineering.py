...
def engineer_features(df: pd.DataFrame) -> pd.DataFrame:
    """
    Offline/Batch feature engineering function on a DataFrame.
    """
    df = df.copy()
    ...
    df['cluster_anomaly_pct'] = (df['spatial_temp_zscore'].abs() > 2.5).astype(float)
    ...
    for col in ['temperature_c', 'humidity_pct', 'pressure_hpa']:
        ...
        if f'{col}_roll24_zscore' not in df.columns:
            df[f'{col}_roll24_zscore'] = (df[f'{col}_roll24_diff'].abs()) / (df[f'{col}_roll24_std'] + 1e-9)
...
def to_model_matrix(data: Union[pd.DataFrame, Dict[str, Any], List[Dict[str, Any]]], feature_cols: list) -> pd.DataFrame:
    """
    Transforms engineered record(s) into the exact 75-feature numeric matrix required by XGBoost and Isolation Forest.
    Handles exact whitespace matching from pipeline_metadata.json.
    """
    ...
    for f in feature_cols:
        ...
        if f in df.columns:
            ...
            X[f] = pd.to_numeric(df[col_map[f_clean]], errors='coerce')
        elif f_clean.startswith('cluster_'):
            cluster_name = f_clean.replace('cluster_', '')
            if 'cluster' in df.columns:
                X[f] = (df['cluster'].astype(str).str.strip() == cluster_name).astype(float)
            else:
                X[f] = 0.0
        else:
            X[f] = 0.0
